...
\textbf{Source:} Journal of Guidance, Control, and Dynamics \\
\textbf{DOI:} \url{https://doi.org/10.2514/2.4611}
\begin{itemize}[noitemsep]
    \item \textbf{Problem statement:} How to compute collision risk rigorously for close approaches.
    \item \textbf{Dataset used:} Cataloged space objects and encounter uncertainty states.
    \item \textbf{Methods applied:} Analytical probability of collision using encounter geometry and covariance.
    \item \textbf{Key findings:} Probability-based thresholds are better than simple geometric screening rules.
\end{itemize}

\subsection*{2) Newman et al. (2009)}
\textbf{Source:} Acta Astronautica \\
\textbf{DOI:} \url{https://doi.org/10.1016/j.actaastro.2009.10.005}
\begin{itemize}[noitemsep]
    \item \textbf{Problem statement:} How to run robotic mission conjunction assessment in operations.
    \item \textbf{Dataset used:} U.S. tracking catalog conjunction products plus mission trajectory data.
    \item \textbf{Methods applied:} Multi-stage screening, escalation, analyst review, and maneuver planning.
    \item \textbf{Key findings:} Layered pipelines reduce false alerts and support practical mission decisions.
\end{itemize}

\subsection*{3) Flohrer et al. (2009)}
\textbf{Source:} Advances in Space Research \\
\textbf{DOI:} \url{https://doi.org/10.1016/j.asr.2009.04.012}
\begin{itemize}[noitemsep]
    \item \textbf{Problem statement:} How to identify and assess high-risk conjunction events for ESA missions.
    \item \textbf{Dataset used:} Daily screenings of tracked objects with orbit prediction and uncertainty data.
    \item \textbf{Methods applied:} Automated filtering, manual refinement, trend checks, and decision gates.
    \item \textbf{Key findings:} Staged assessment improves safety while limiting unnecessary maneuvers.
\end{itemize}

\subsection*{4) Braun et al. (2016)}
\textbf{Source:} CEAS Space Journal \\
\textbf{DOI:} \url{https://doi.org/10.1007/s12567-016-0119-3}
\begin{itemize}[noitemsep]
    \item \textbf{Problem statement:} How to scale operational collision support across many missions.
    \item \textbf{Dataset used:} Operational conjunction streams, OD updates, and mission constraints.
    \item \textbf{Methods applied:} Standardized support workflow and mission-tailored risk handling.
    \item \textbf{Key findings:} Institutional support improves consistency, but upstream uncertainty remains a major limiter.
\end{itemize}

\subsection*{5) Weigel et al. (2024)}
\textbf{Source:} Journal of Guidance, Control, and Dynamics \\
\textbf{DOI:} \url{https://doi.org/10.2514/1.G008245}
\begin{itemize}[noitemsep]
    \item \textbf{Problem statement:} How to make covariance models realistic enough for robust CA decisions.
    \item \textbf{Dataset used:} Historical mission orbit-estimation and covariance behavior data.
    \item \textbf{Methods applied:} Consider-covariance calibration and comparative risk-ranking experiments.
    \item \textbf{Key findings:} Better covariance realism improves event ranking and maneuver timing quality.
\end{itemize}
